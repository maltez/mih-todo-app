...
const taskTemplate = require('./activity.server.model').taskTemplate;
const eventTemplate = require('./activity.server.model').eventTemplate;
...
const WorkDay = function (dayName, dayIndex) {
  return {
    start: {
      type    : String,
      'default' : '09:00',
...
UserSchema.pre('save', function (next) {
  ...
  if (
    this.password && !/==/.test(this.password) //Check, if password wasn't hashed
  ) {
